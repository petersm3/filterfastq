...
* <code>filterfastq.pl</code> uses NCBI's command-line [BLAST](https://blast.ncbi.nlm.nih.gov/doc/blast-help/index.html) (<code>blastn</code>) to compare sequences in a serial fashion (even using multiple threads, i.e., one set of sequences compared via one <code>blastn</code> job), which can take a long time
